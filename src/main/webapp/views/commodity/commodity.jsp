...
<div class="container">
    <div class="content" style="margin: auto">
        <div class="information-title">
            ...
            <c:choose>
                <c:when test="${sessionScope.admin}">
                    <div class="bottom fr">
                        <a href="<c:url value="/commodity/commodityUpdate"/>">添加商品</a>
                    </div>
                </c:when>
            </c:choose>
        </div>
        ...
        <div class="search bar1" align="center">
            <form action="<c:url value="/commodity/list"/>">
                ...
                <c:forEach items="${requestScope.list}" var="commodity">
                    <tr>
                        ...
                        <c:choose>
                            <c:when test="${sessionScope.admin}">
                                <td class="operate">
                                    <a href="<c:url value="/commodity/queryById/${commodity.commodity_id}"/>" class="table-view">修改</a>
                                    <a>/</a>
                            ...
                            <c:otherwise>
                                <td class="operate">
                                    <a href="JavaScript:buyingCommodity('${commodity.commodity_id}','${sessionScope.loginUserId}')" class="table-view">购买</a>
                                </td>
                            </c:otherwise>
                        </c:choose>
                    </tr>
...
<script>
    function deleteCommodity(commodity_id){
        if(window.confirm("确定要删除id为"+commodity_id+"的商品吗？")){
            location.href = "<c:url value='/commodity'/>?type=delete&commodity_id="+commodity_id;
        }
    }
    function buyingCommodity(commodity_id,user_id){
        ...
        if (quantity != null) {
            ...
            if(window.confirm("确定要购买id为"+commodity_id+"的商品吗？")){
                location.href = "<c:url value='/commodity'/>?type=buying&commodity_id=" + commodity_id + "&count=" + quantity + "&user_id=" + user_id;
            }
        } else {
            // 用户点击了取消按钮
            alert("取消购买");
